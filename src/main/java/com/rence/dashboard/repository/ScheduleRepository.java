...
//	public List<ScheduleEntity> backoffice_schedule_list_exist_off(String backoffice_no, String not_stime, String not_etime);
	...
	// 휴무 일정
	@Query(nativeQuery = true, value = "select * from roomschedule where backoffice_no=?1 and extract (month from not_stime) in (select extract (month from systimestamp) from dual) order by not_stime")
