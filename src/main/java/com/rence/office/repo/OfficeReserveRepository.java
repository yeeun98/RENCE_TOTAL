...
public interface OfficeReserveRepository extends JpaRepository<OfficeReserveEntity, Object> {
	...
	public List<OfficeReserveEntity> select_all_reserve(String backoffice_no, String room_no, String reserve_stime);

	@Query(nativeQuery = true, 
			value = "select * from reserveinfo where backoffice_no=?1 and room_no=?2 and reserve_state!='cancel' order by reserve_no asc")
	public List<OfficeReserveEntity> select_all_reserve_office(String backoffice_no, String room_no);
	
	@Query(nativeQuery = true, value = "select * from (" + "select * " + "from reserveinfo "
			+ "where user_no=?1 order by reserve_no desc)" + "where rownum=1")
	public OfficeReserveEntity select_one_reserve_no(String user_no);
