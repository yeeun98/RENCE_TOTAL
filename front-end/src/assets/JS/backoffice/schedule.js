...
$(() => {
  $("input:radio[name='set_schedule']:eq(0)").click(() => {
    $('.off-type-warning:eq(0)').removeClass('blind');
    $('.off-type-warning:eq(1)').addClass('blind');
  });

  $("input:radio[name='set_schedule']:eq(1)").click(() => {
  ...
  $('.type-border-txt.time-input:eq(0)').change(() => {
    $("input:checkbox[name='select-room']").prop('checked', false);
  });

  $('.type-border-txt.time-input:eq(1)').change(() => {
    $("input:checkbox[name='select-room']").prop('checked', false);
  });
    ...
    if ($("input:radio[name='set_schedule']").is(':checked')) {
      if ($('.time-input:eq(0)').val() != '' && $('.time-input:eq(1)').val() != '') {
        if ($('.time-input:eq(0)').val() > $('.time-input:eq(1)').val()) {
          ...
          const sDateTime = $('.time-input:eq(0)').val().split(' ');
          const eDateTime = $('.time-input:eq(1)').val().split(' ');
          ...
          $.ajax({
            ...
            success(res) {
              ...
              if (res.cnt > 0) {
                const empty_row = $($('.ct-body-row')[0]).clone();
                $('.ct-body').empty();
                $('.ct-body').append(empty_row);

                for (let i = 0; i < res.cnt; i++) {
                  const body_row = $($('.ct-body-row')[0]).clone();
  ...
  $('.reservation-ct').scroll(function () {
    if (Math.ceil($(this).scrollTop() + $(this).innerHeight()) >= $(this).prop('scrollHeight')) {
      if ($('.ct-body-row').length < Number($('#maxCnt').attr('maxCnt')) && scroll_flag2) {
        ...
        const backoffice_no = $.cookie('backoffice_no');

        const room_no = window.location.href.split('&room_no=')[1].split('&')[0];

        const not_sdate = window.location.href.split('&not_sdate=')[1].split('&')[0];
        const not_stime = window.location.href.split('&not_stime=')[1].split('&')[0];
        const not_edate = window.location.href.split('&not_edate=')[1].split('&')[0];
        const not_etime = window.location.href.split('&not_etime=')[1].split('&')[0];
        const off_type = window.location.href.split('&off_type=')[1].split('&')[0];
  ...
  $('.ct-body').on('click', '.reserve_cnt', function () {
    ...
    const not_edate = eDateTime[0];
    const not_etime = eDateTime[1];
    const room_no = $(this).parent().siblings('.ct-body-cell:eq(0)').children()
      .attr('room_no');
    const off_type = $("input:radio[name='set_schedule']:checked").val();
    const reserve_cnt = parseInt($(this).attr('reserve_cnt'));
  ...
  // 일정 설정
  let check_room = '';
  $(document).on('click', '#schedule-confirm-btn', () => {
    console.log('in');
    ...
    if (check_room.length != 0) {
      ...
      for (let i = 0; i < check_room.length; i++) {
        const room_no = $(check_room[i]).find('#room_no').attr('room_no');
  ...
  // 팝업
  $('#radio-check-closeBtn').click(() => {
    $('#radio-check-popup').addClass('blind');
    $('.popup-background:eq(0)').addClass('blind');
  });

  $('#time-input-closeBtn').click(() => {
    $('#time-input-popup').addClass('blind');
    $('.popup-background:eq(0)').addClass('blind');
  });

  $('#no-reservation-closeBtn').click(() => {
  ...
  $('#btn-reserve-cancel').on('click', () => {
    ...
    if (check_arr.length == 0) {
      ...
      for (let i = 0; i < check_arr.length; i++) {
        const s_date = $(check_arr[i]).find('.reserve-stime').text().split(' ~ ')[0];
  ...
  $('#btn-dayoff-calendar').click(() => {
    ...
    $.ajax({
      ...
      success(res) {
        ...
        if (res.cnt > 0) {
          const empty_item = $($('.dayoff-list-item')[0]).clone();
          $('.dayoff-list').empty();
          $('.dayoff-list').append(empty_item);

          for (let i = 0; i < res.cnt; i++) {
            const dayoff_list_item = $($('.dayoff-list-item')[0]).clone();
  ...
  // 휴무 취소 확인 팝업 - 닫기
  $('#dayoff-cancel-confirm-closeBtn').click(() => {
    $('.popup-background:eq(1)').addClass('blind');
    $('#dayoff-cancel-popup').addClass('blind');
  });

  // 휴무 취소 확인 팝업 - 휴무 취소
  $('#dayoff-cancel-confirm-btn').click(() => {
    // 로딩 화면 열기
    $('.popup-background:eq(1)').removeClass('blind');
    $('#spinner-section').removeClass('blind');
  ...
  $('#dayoff-cancel-confirmOK-btn').click(() => {
    $('#dayoff-cancel-confirmOK-popup').addClass('blind');
    $('.popup-background:eq(1)').addClass('blind');
  });

  $('#dayoff-cancel-fail-btn').click(() => {
